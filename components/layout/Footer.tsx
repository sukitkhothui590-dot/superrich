import React from "react";
import Link from "next/link";

const sitemapLinks = [
  { href: "/", label: "หน้าแรก" },
  { href: "/branches", label: "สาขาทั้งหมด" },
  { href: "/news", label: "ข่าวสาร" },
  { href: "/contact", label: "ติดต่อเรา" },
];

const exchangeLinks = [
  { href: "/exchange-rate", label: "อัตราแลกเปลี่ยน" },
  { href: "#", label: "Trip Budget Guide" },
];

const serviceLinks = [
  { href: "/services", label: "SPR Money Exchange" },
  { href: "/services", label: "SPR Online Booking" },
  { href: "/services", label: "SPR Money Transfer" },
];

const contactLinks = [
  { href: "#", label: "เกี่ยวกับ SuperRich" },
  { href: "#", label: "คำถามที่พบบ่อย" },
  { href: "#", label: "เงื่อนไขและนโยบาย" },
];

function SocialIcon({ children, href = "#", label }: { children: React.ReactNode; href?: string; label: string }) {
  return (
    <a
      href={href}
      target="_blank"
      rel="noopener noreferrer"
      aria-label={label}
      className="w-9 h-9 rounded-lg border border-white/40 bg-[#0F766E] flex items-center justify-center text-white hover:bg-[#0B5A53] transition-colors"
    >
      {children}
    </a>
  );
}

export default function Footer() {
  return (
    <footer>
      {/* Upper section - white */}
      <div className="bg-white">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-12 lg:py-16">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-8">
            {/* Logo + Address + App */}
            <div className="lg:col-span-2">
              <div className="flex items-center gap-2.5 mb-4">
                <div className="w-10 h-10 rounded-full bg-[#0F766E] flex items-center justify-center shrink-0">
                  <span className="text-white font-black text-sm">SPR</span>
                </div>
                <div>
                  <div className="text-surface-700 font-bold text-base">SuperRich Currency Exchange</div>
                  <div className="text-surface-700/70 text-xs">SINCE 1965</div>
                </div>
              </div>
              <p className="text-sm text-surface-700/80 leading-relaxed mb-5">
                เลขที่ 491/5-491/7 ถนนสีลม แขวงสีลม เขตบางรัก กรุงเทพฯ 10500
              </p>
              {/* Removed app download buttons per latest design */}
            </div>

            {/* แผนที่เว็บไซต์ */}
            <div>
              <h3 className="text-sm font-bold text-surface-700 mb-4">แผนที่เว็บไซต์</h3>
              <ul className="space-y-2.5">
                {sitemapLinks.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-sm text-surface-700/80 hover:text-[#0F766E] transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* อัตราแลกเปลี่ยน */}
            <div>
              <h3 className="text-sm font-bold text-surface-700 mb-4">อัตราแลกเปลี่ยน</h3>
              <ul className="space-y-2.5">
                {exchangeLinks.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-sm text-surface-700/80 hover:text-[#0F766E] transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* บริการของเรา */}
            <div>
              <h3 className="text-sm font-bold text-surface-700 mb-4">บริการของเรา</h3>
              <ul className="space-y-2.5">
                {serviceLinks.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-sm text-surface-700/80 hover:text-[#0F766E] transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* ติดต่อเรา */}
            <div>
              <h3 className="text-sm font-bold text-surface-700 mb-4">ติดต่อเรา</h3>
              <ul className="space-y-2.5">
                {contactLinks.map((link) => (
                  <li key={link.label}>
                    <Link href={link.href} className="text-sm text-surface-700/80 hover:text-[#0F766E] transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>
      </div>

      {/* Lower bar - orange */}
      <div className="bg-[#0F766E]">
        <div className="max-w-7xl mx-auto px-4 lg:px-6 py-4">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-white text-center md:text-left">
              Copyright &copy; 2025 Super Rich | All Rights Reserved | Terms and Conditions | ใบอนุญาตประกอบธุรกิจ MC125610007
            </p>
            <div className="flex items-center gap-2">
              <SocialIcon href="https://line.me" label="LINE">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M24 10.304C24 4.614 18.627.2 12 .2S0 4.614 0 10.304c0 5.003 4.434 9.192 10.424 9.984.406.088.96.268 1.1.616.126.316.082.81.04 1.128l-.178 1.068c-.054.326-.254 1.276 1.118.696 1.372-.58 7.396-4.356 10.09-7.454C24.22 14.466 24 12.432 24 10.304z" />
                </svg>
              </SocialIcon>
              <SocialIcon href="https://facebook.com" label="Facebook">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </SocialIcon>
              <SocialIcon href="https://instagram.com" label="Instagram">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </SocialIcon>
              <SocialIcon href="https://tiktok.com" label="TikTok">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z" />
                </svg>
              </SocialIcon>
              <SocialIcon href="https://youtube.com" label="YouTube">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                </svg>
              </SocialIcon>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
